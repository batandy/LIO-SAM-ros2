...
public:
    ...
    */
    void imuOdometryHandler(const nav_msgs::msg::Odometry::SharedPtr odomMsg)
    {
        ...
        // 현재 시간의 IMU 오도메트리 포즈 = 최근 라이다 오도메트리 포즈 *  IMU 오도메트리 변화량 포즈 변환
...
public:
    ...
    */
            ...
            ParamServer("lio_sam_imu_preintegration", options)
    {
        ...
        // IMU 원시 데이터를 구독하고 아래의 그래프 최적화 결과를 사용하여 두 프레임 간의 IMU preintegration을 적용하여 
        // 매 시간 (IMU 주파수)의 IMU 오도메트리를 추정
        subImu = create_subscription<sensor_msgs::msg::Imu>(
            imuTopic, qos_imu,
            std::bind(&IMUPreintegration::imuHandler, this, std::placeholders::_1),
            imuOpt);

        // Map 최적화에서 온 라이다 오도메트리를 구독하고 두 프레임 간의 IMU Preintegration을 사용하여 그래프를 구축하고 
        // 현재 프레임 pose를 최적화 (이 Pose는 IMU 오도메트리를 업데이트하고 다음 번 그래프 최적화에만 사용)
        // optimization을 하기 전까지 IMU data를 저장할 뿐 prediction은 하지 않음
    ...
    */
    //  LIO-SAM을 실행시켰을 때 ImageProjection→FeatureExtraction→MapOptimization의 일련의 과정을 거친 후에 생성되는 
    // world 좌표계 기준의 keyframe의 pose를 콜백으로 받아온다. 
    void odometryHandler(const nav_msgs::msg::Odometry::SharedPtr odomMsg)
    {
        ...
        // preintegration은 i번째 keyframe과 j번째 keyframe 사이의 measuremets값을 하나의 factor로 만든다. 즉 optimization을 하기 위해서는
        // 적어도 2개의 pose가 필요함. 그래서 첫 pose가 왔을때 초기 값을 세팅후 return
        ...
        // 이전 프레임과 현재 프레임 간의 IMU preintegration을 계산하고 이전 상태에 integration 값을 
        // 적용하여 현재 프레임의 초기 상태를 추정
        ...
        while (!imuQueOpt.empty())
        {
            ...
            // 현재 IMU 데이터의 시간이 최신의 최적화 시간보다 이전인지 확인합니다. 
            // 이전이라면, 두 최적화 사이의 IMU 데이터임을 의미합니다.
        ...
        // 변수 노드에 초기 값 할당  X: 자세, V: 속도, B: 바이어스
        ...
        // 2. 최적화 이후,  re-propagate 수행, 최적화는 IMU bias를 업데이트하며, 최신 bias를 사용하여 
        // 현재 라이다 오도메트리 시점 이후 IMU preintegration을 다시 계산하고, 이 값은 각 시점의 포즈 계산에 사용
    ...
    */
    ...
    /**
     * IMU 원본 데이터를 구독
     * 1. 이전 프레임 라이다 오도메트리 타임스탬프에 해당하는 상태와 bias를 사용하여 이 시점부터 현재 시점까지의 
     *    IMU preintegraion을 적용하여 현재 시점의 상태를 얻음. (IMU 오도메트리)
    ...
    */
    void imuHandler(const sensor_msgs::msg::Imu::SharedPtr imu_raw)
    {
        ...
        // 이전 IMU factor 그래프 최적화가 성공적으로 수행되었는지 확인하여, 
        // 이전 프레임(라이다 오도메트리 프레임)의 상태와 bias가 업데이트되었고, preintegraion이 다시 계산되었음을 보장
        // 이 과정을 하지 않으면 부정확한 bias로 추정을 하게 되어 결과가 매우 부정확해진다.
        if (doneFirstOpt == false)
            return;

        //현재 imu와 이전 측정값 사이의 시간 간격 계산
        double imuTime = stamp2Sec(thisImu.header.stamp);
        double dt = (lastImuT_imu < 0) ? (1.0 / 500.0) : (imuTime - lastImuT_imu);
        lastImuT_imu = imuTime;

        // integrate this single imu message
        // IMU preintegration에 현재 IMU 데이터를 추가 (주의: 이 IMU preintegraion의 시작 시간은 
        // 이전 라이다 오도메트리 프레임의 타임스탬프 입니다.)
        // 이 함수를 호출하면 IMU 적분기는 제공된 가속도, 각속도, 시간 정보를 이용하여
        // IMU의 상태(위치, 자세, 속도)를 예측하고 업데이트합니다.
        imuIntegratorImu_->integrateMeasurement(gtsam::Vector3(thisImu.linear_acceleration.x, thisImu.linear_acceleration.y, thisImu.linear_acceleration.z),
                                                gtsam::Vector3(thisImu.angular_velocity.x,    thisImu.angular_velocity.y,    thisImu.angular_velocity.z), dt);
        ...
        // predict odometry
        // 이전 라이다 오도메트리 프레임의 상태와 바이어스를 사용하여 이 시점부터 현재 시점까지의 
        // IMU preintegraion을 적용하여 현재 시점의 상태를 얻음
        ...
        // 라이다 좌표계로 변환
        // currentState를 바탕으로 LiDAR 센서의 위치와 자세를 계산합니다
        ...
        // 계산된 LiDAR 센서의 위치와 자세, 선속도, 각속도를 publisth
