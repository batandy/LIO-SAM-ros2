...
/**
1. ImageProjection 생성자를 통해 구독자, 발행자, 콜백 그룹 등을 초기화합니다.
2. cloudHandler 콜백 함수가 LiDAR 데이터 메시지를 수신합니다.
3. cachePointCloud 함수에서 LiDAR 데이터를 큐에 쌓고 가장 오래된 데이터를 현재 프레임으로 설정합니다.
4. deskewInfo 함수에서 현재 프레임의 시작, 끝 시간에 해당하는 IMU와 Odometry 데이터를 처리합니다.
5. imuDeskewInfo 함수에서 현재 프레임의 초기 자세를 추정합니다.
6. odomDeskewInfo 함수에서 초기 위치를 추정합니다.
7. projectPointCloud 함수에서 LiDAR 포인트의 모션 왜곡을 보정합니다.
8. cloudExtraction 함수에서 유효한 포인트를 추출합니다.
9. publishClouds 함수에서 결과 포인트 클라우드를 게시합니다.
10. resetParameters 함수로 매개변수와 버퍼를 초기화합니다.
이러한 과정이 매 LiDAR 프레임마다 반복 실행됩니다.
...
// ImageProjection 생성자를 통해서 구독자. 발행자, 콜백 그룹 등을 초기화 하며 main문에서 제일 먼저 실행됨.
public:
            ...
            ParamServer("lio_sam_imageProjection", options), deskewFlag(0)
    {
        ...
        subImu = create_subscription<sensor_msgs::msg::Imu>(
            imuTopic, qos_imu,      //구독할 토픽 이름, qos 프로파일
        ...
        // 현재 LiDAR 프레임의 motion distrotion 보정 후 포인트 클라우드 정보 publish. 현재 라이다 프레임의 관련 정보들 저장함, 여기에도 동일하게 보정된 라이다 토픽이 존재
    ...
    */
    void imuHandler(const sensor_msgs::msg::Imu::SharedPtr imuMsg)
    {
        ...
        // 멀티쓰레드를 사용하기 때문에 해당 큐에 데이터를 추가하는 동안은 다른 스레드가 큐에 접근을 막음
    ...
    */
    void cloudHandler(const sensor_msgs::msg::PointCloud2::SharedPtr laserCloudMsg)
    {
        ...
        // 2)각 포인트 인덱스 정렬
        // 3) 라이다 포인트의 motion distortion을 보정하고 포인트를 저장
        projectPointCloud();
    ...
    */
    bool cachePointCloud(const sensor_msgs::msg::PointCloud2::SharedPtr& laserCloudMsg)
    {
        ...
        //*즉 true일때 모든 포인트가 유효한 데이터를 가지고 있음을 나타냄
    ...
    void imuDeskewInfo()
    {
        ...
        for (int i = 0; i < (int)imuQueue.size(); ++i)
        {
            ...
            if (currentImuTime <= timeScanCur)
                imuRPY2rosRPY(&thisImuMsg, &cloudInfo.imu_roll_init, &cloudInfo.imu_pitch_init, &cloudInfo.imu_yaw_init);  //변환 결과를 imu_RPY_init에 저장
    ...
    */
    void odomDeskewInfo()
    {
        ...
        // 두 프레임간의 상대 변환 행렬을 계산함.
        Eigen::Affine3f transBt = transBegin.inverse() * transEnd;

        // 상대적인 변환에서 이동 및 회전 변화량을 추출 (오일러 각도로) R|T
    ...
    */
    void projectPointCloud()
    {
        ...
        for (int i = 0; i < cloudSize; ++i)
        {
            ...
            // 수평 스캔 각도 단계 (한바퀴 스캔: 1024번, 스캔 간격: ?)
            //링별 포인트 정렬을 위해 새로 인덱싱
    ...
    */
    void cloudExtraction()
    {
        ...
        for (int i = 0; i < N_SCAN; ++i)
        {
            ...
            // 현재 링 스캔 시작 포인트 인덱스 저장, 시작과 끝부분에서 잡음이 있는 포인트 제거 위해
            cloudInfo.start_ring_index[i] = count - 1 + 5;
            for (int j = 0; j < Horizon_SCAN; ++j)
            {
                // 유효한 라이다 포인트. 라이다에서 측정되지 않은 포인트는 FLT_MAX값을 가짐
