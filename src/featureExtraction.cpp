...
public:
    ...
    */
    void laserCloudInfoHandler(const lio_sam::msg::CloudInfo::SharedPtr msgIn)
    {
        ...
        // 각 포인트의 곡률을 계산 이때 앞뒤 5개 포인트는 제외
        calculateSmoothness();
    ...
    */
    ...
    {   
        // 현재 포인트의 앞뒤 5개를 제외한 나머지 포인트에 대해 곡률 계산
        int cloudSize = extractedCloud->points.size();
        for (int i = 5; i < cloudSize - 5; i++)
        {
            //현재 포인트 i를 중심으로 양쪽으로 5개의 포인트까지 포함하여 거리 차이를 계산
            //평탄한 표면에서는 거리 차이가 작고, 곡면에서는 거리 차이가 크다
     ...
     * markOccludedPoints() 함수는 추출된 포인트 클라우드에서 가려진(occluded) 포인트를 탐지하고 표시하는 역할을 합니다. 
     * 가려진 포인트는 주변 포인트들과 깊이 값의 차이가 크기 때문에 발생합니다.
    */
    void markOccludedPoints()
    {
        ...
        for (int i = 5; i < cloudSize - 6; ++i)
        {
            ...
            // 양 옆 포인트의 거리 차이가 2%이상이면 가려진 포인트로 표시
            //가까운 거리에서는 2%의 차이가 0.3보다 작기 때문에 또 한번 더 적용
        ...
        for (int i = 0; i < N_SCAN; i++)
        {
            ...
            for (int j = 0; j < 6; j++)
            {
                ...
                for (int k = ep; k >= sp; k--)
                {
                    ...
                    // 현재 포인트가 가려지지 않거나 처리되지 않고, 곡률이 edgeThreshold 보다 크면 코너로 간주합니다.
                ...
                for (int k = sp; k <= ep; k++)
                {
                    //코너는 cloudLabel=1, 평면은 cloudLbale=-1
                    if (cloudLabel[k] <= 0){
